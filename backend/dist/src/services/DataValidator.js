...
class DataValidator {
    ...
    performSchemaValidation(data, schema, errors) {
    ...
    verifyAndConvertTypes(data, schema, errors, validatedData) {
        for (const key in schema.properties) {
            if (key in data) {
    ...
    checkRequiredFields(data, schema, errors) {
        if (schema.required) {
    ...
    calculateQualityScore(data, schema, errors) {
        let score = 100;
