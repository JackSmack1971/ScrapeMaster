...
describe('PaginationDetector', () => {
  ...
  beforeEach(() => {
    ...
    mockPage = {
      ...
      evaluate: jest.fn((fn: any, element: unknown) => { // Changed type to unknown
  ...
  it('should detect numbered pagination when present', async () => {
    ...
    expect(detectedPattern?.type).toBe('numbered');
    expect(detectedPattern?.name).toContain('Numbered Pagination');
  });
  ...
  it('should detect next/prev pagination when present', async () => {
    ...
    expect(detectedPattern).not.toBeNull();
    expect(detectedPattern?.type).toBe('next-prev');
    expect(detectedPattern?.name).toContain('Next Button');
  });
  ...
  it('should prioritize patterns with higher confidence', async () => {
    ...
    mockPage.evaluate.mockImplementationOnce((fn: any, element: unknown) => { // Changed type to unknown
      if (fn.toString().includes('el.textContent')) {
    ...
    expect(detectedPattern).not.toBeNull();
    expect(detectedPattern?.name).toContain('Numbered Pagination (Common)'); // Expect the higher confidence one
  });
  ...
  it('should add a custom pattern and prioritize it if confidence is higher', async () => {
    ...
    mockPage.evaluate.mockImplementation((fn: any, element: unknown) => { // Changed type to unknown
      if (fn.toString().includes('el.textContent')) {
    ...
    expect(detectedPattern).not.toBeNull();
    expect(detectedPattern?.name).toBe('Custom High Confidence Next');
  });
  ...
  it('should reduce confidence if text pattern does not match', async () => {
    ...
    mockPage.evaluate.mockImplementation((fn: any, element: any) => {
      if (fn.toString().includes('el.textContent')) {
    ...
    expect(detectedPattern).not.toBeNull();
    expect(detectedPattern?.name).toBe('Next Button (Common)');
    expect(detectedPattern?.confidence).toBeCloseTo(0.9 * 0.5); // Original confidence * 0.5
